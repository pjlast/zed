...
impl InlineCompletionProvider for CodyCompletionProvider {
    fn is_enabled(
        ...
        settings.cody_enabled(language.as_ref(), file.map(|f| f.path().as_ref()))
    }
    ...
    fn discard(&mut self, cx: &mut ModelContext<Self>) {
        ...
        if !settings.cody.feature_enabled {
            return;
        }

        self.cody
            .update(cx, |cody, cx| {
                cody.discard_completions(&self.completions, cx)
            })
            .detach_and_log_err(cx);
        if let Some(telemetry) = self.telemetry.as_ref() {
            telemetry.report_cody_event(None, false, self.file_extension.clone());
        }
    }
...
mod tests {
    ...
    async fn test_cody(executor: BackgroundExecutor, cx: &mut TestAppContext) {
        // flaky
        init_test(cx, |_| {});

        let (cody, cody_lsp) = Cody::fake(cx);
        ...
        .await;
        let cody_provider = cx.new_model(|_| CodyCompletionProvider::new(cody));
        cx.update_editor(|editor, cx| editor.set_inline_completion_provider(cody_provider, cx));

        // When inserting, ensure autocompletion is favored over Cody suggestions.
        cx.set_state(indoc! {"
            oneˇ
            two
            three
        ...
        handle_cody_completion_request(
            &cody_lsp,
            vec![cody::request::Completion {
                text: "one.cody1".into(),
        ...
        executor.advance_clock(CODY_DEBOUNCE_TIMEOUT);
        cx.update_editor(|editor, cx| {
            assert!(editor.context_menu_visible());
            assert!(!editor.has_active_inline_completion(cx));

            // Confirming a completion inserts it and hides the context menu, without showing
            // the cody suggestion afterwards.
            editor
        ...
        // Ensure Cody suggestions are shown right away if no autocompletion is available.
        cx.set_state(indoc! {"
            oneˇ
            two
            three
        ...
        cx.update_editor(|editor, cx| {
            assert!(!editor.context_menu_visible());
            assert!(editor.has_active_inline_completion(cx));
            assert_eq!(editor.display_text(cx), "one.cody1\ntwo\nthree\n");
            assert_eq!(editor.text(cx), "one.\ntwo\nthree\n");
        });

        // Reset editor, and ensure autocompletion is still favored over Cody suggestions.
        cx.set_state(indoc! {"
            oneˇ
            two
            three
        ...
        cx.update_editor(|editor, cx| {
            ...
            // When hiding the context menu, the Cody suggestion becomes visible.
        ...
        // After debouncing, new Cody completions should be requested.
        handle_cody_completion_request(
            &cody_lsp,
            vec![cody::request::Completion {
                text: "one.cody2".into(),
        ...
        cx.update_editor(|editor, cx| {
            assert!(!editor.context_menu_visible());
            assert!(editor.has_active_inline_completion(cx));
            assert_eq!(editor.display_text(cx), "one.cody2\ntwo\nthree\n");
            assert_eq!(editor.text(cx), "one.c\ntwo\nthree\n");

            // Canceling should remove the active Cody suggestion.
            editor.cancel(&Default::default(), cx);
            assert!(!editor.has_active_inline_completion(cx));
        ...
        cx.update_editor(|editor, cx| {
            ...
            assert!(!editor.has_active_inline_completion(cx));
            assert_eq!(editor.display_text(cx), "one.cody2\ntwo\nthree\n");
            assert_eq!(editor.text(cx), "one.cody2\ntwo\nthree\n");
    ...
    #[gpui::test(iterations = 10)]
    async fn test_accept_partial_cody_suggestion(
        ...
        cx.update_editor(|editor, cx| {
            ...
            assert!(editor.has_active_inline_completion(cx));
            assert_eq!(editor.text(cx), "one.cody\ntwo\nthree\n");
            assert_eq!(editor.display_text(cx), "one.cody1\ntwo\nthree\n");

            // Accepting next word should accept the non-word and cody suggestion should be gone
            editor.accept_partial_inline_completion(&Default::default(), cx);
            assert!(!editor.has_active_inline_completion(cx));
            assert_eq!(editor.text(cx), "one.cody1\ntwo\nthree\n");
            assert_eq!(editor.display_text(cx), "one.cody1\ntwo\nthree\n");
        });
        ...
        cx.update_editor(|editor, cx| {
            ...
            // Accepting next word should accept the next word and cody suggestion should still exist
            editor.accept_partial_inline_completion(&Default::default(), cx);
            assert!(editor.has_active_inline_completion(cx));
            assert_eq!(editor.text(cx), "one.123. cody\ntwo\nthree\n");
            assert_eq!(
                editor.display_text(cx),
                "one.123. cody\n 456\ntwo\nthree\n"
            );

            // Accepting the whitespace should accept the non-word/whitespaces with newline and cody suggestion should be gone
            editor.accept_partial_inline_completion(&Default::default(), cx);
            assert!(!editor.has_active_inline_completion(cx));
            assert_eq!(editor.text(cx), "one.123. cody\n 456\ntwo\nthree\n");
            assert_eq!(
    ...
    #[gpui::test]
    async fn test_cody_completion_invalidation(
        executor: BackgroundExecutor,
        cx: &mut TestAppContext,
    ) {
        init_test(cx, |_| {});
    ...
    #[gpui::test]
    async fn test_cody_multibuffer(executor: BackgroundExecutor, cx: &mut TestAppContext) {
        init_test(cx, |_| {});
        ...
        editor
            .update(cx, |editor, cx| {
                editor.set_inline_completion_provider(cody_provider, cx)
            })
            .unwrap();
        ...
        _ = editor.update(cx, |editor, cx| {
            // Ensure cody suggestions are shown for the first excerpt.
            editor.change_selections(None, cx, |s| {
    ...
    #[gpui::test]
    async fn test_cody_disabled_globs(executor: BackgroundExecutor, cx: &mut TestAppContext) {
        ...
        let mut cody_requests = cody_lsp
            .handle_request::<cody::request::GetCompletions, _, _>(
                move |_params, _cx| async move {
                    Ok(cody::request::GetCompletionsResult {
                        completions: vec![cody::request::Completion {
                            text: "next line".into(),
                            range: lsp::Range::new(
        ...
        executor.advance_clock(CODY_DEBOUNCE_TIMEOUT);
        assert!(cody_requests.try_next().is_err());
        ...
        executor.advance_clock(CODY_DEBOUNCE_TIMEOUT);
        assert!(cody_requests.try_next().is_ok());
    }

    fn handle_cody_completion_request(
        lsp: &lsp::FakeLanguageServer,
        completions: Vec<cody::request::Completion>,
        completions_cycling: Vec<cody::request::Completion>,
    ) {
        lsp.handle_request::<cody::request::GetCompletions, _, _>(move |_params, _cx| {
        ...
        lsp.handle_request::<cody::request::GetCompletionsCycling, _, _>(move |_params, _cx| {
