use cody::{request::PromptUserDeviceFlow, Cody, Status};
use gpui::{
...
const CODY_SIGN_UP_URL: &str = "https://github.com/features/cody";
...
impl CodyCodeVerification {
    ...
    fn render_prompting_modal(
        ...
        v_flex()
            ...
            .gap_2()
            .items_center()
            .child(Headline::new("Use GitHub Cody in Zed.").size(HeadlineSize::Large))
            .child(
                Label::new("Using Cody requires an active subscription on GitHub.")
                    .color(Color::Muted),
            )
    ...
    fn render_unauthorized_modal(cx: &mut ViewContext<Self>) -> impl Element {
        v_flex()
            .child(Headline::new("You must have an active GitHub Cody subscription.").size(HeadlineSize::Large))

            .child(Label::new(
                "You can enable Cody by connecting your existing license once you have subscribed or renewed your subscription.",
            ).color(Color::Warning))
            .child(
                Button::new("cody-subscribe-button", "Subscribe on GitHub")
                    .full_width()
